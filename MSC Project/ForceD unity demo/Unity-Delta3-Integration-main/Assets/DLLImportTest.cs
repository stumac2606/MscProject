...
public class DLLImportTest : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //StartCoroutine(ToggleRandomForceCoroutine());
    ...
    private void FixedUpdate()
    {
        if (DeviceStatus == deviceStatus.DELTA_OPEN)
        {
            if (GetDHDPosition() >= 0) //check to see if information from haptic device successful
            {
                ...
                //TargetSphere.transform.position = DhdPosition;
            }
            ...
            if (dhdGetPosition(ref px1, ref py1, ref pz1, defaultId) >= 0)
            {
                ...
                Vector3 scaledHapticPosition = new Vector3((float)(py1 * 100), (float)(pz1 * 100), (float)(-px1 * 100));

                // Update the position of the EndEffector
                EndEffector.transform.position = scaledHapticPosition;
            }

            /*// Applying force towards TargetSphere 
            if (forcesOn && applyingRegularForce)
            {
            ...
            /*//Apply resistive force when inside the sphere for specified time 
            if (IsInsideTargetSphereArea() == true)
            {
                // Increment the time inside the TargetSphereArea
                timeInsideTargetSphere += Time.deltaTime;

                // Check if the time is greater than 5 seconds
                if (timeInsideTargetSphere >= timeInsideSphereForForce && !applyingRandomForce)
                {
                    // Start applying the random force
                    ProduceRandomForce();
                }
            }
            else
            {
                // Reset the time when the EndEffector leaves the TargetSphereArea
                timeInsideTargetSphere = 0.0f;
            }*/
    ...
    /*IEnumerator ToggleRandomForceCoroutine()
    {
        while (true)
        {
            // Wait for 10 seconds
            ...
            // Turn off other forces
            applyingRegularForce = false;

            // Toggle the random force on/off
            isRandomForceOn = !isRandomForceOn;

            if (isRandomForceOn)
            {
                // If random force is turned on, start applying random force
                ProduceRandomForce();
            }
            else
            {
                // If random force is turned off, stop applying the force
                ApplyForceToHapticDevice(Vector3.zero);
                applyingRegularForce = true;
                timeSinceRandomForceStart = 0.0f;
            }
